"""
Zeabur 部署：Flask Web 服务
- /  → index.html 任务提交页
- /api/action → 任务确认、完成、列表
"""
import html
import os
...
def confirm_html(task_id: str, task_content: str = "", assignee: str = "") -> str:
    ...
    confirm_url = f"/api/action?id={html.escape(task_id)}&confirm=1"
...
def list_html(assignee: str, tasks: list) -> str:
    items = "".join(f"<li>{html.escape(t['task_content'])}</li>" for t in tasks)
    return f"""<!DOCTYPE html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
...
def action():
    ...
    if task_id:
        ...
        if not task:
            return "<h2>任务不存在</h2>", 400
        content = task.get("task_content", "")
        task_assignee = task.get("assignee", "")
        if confirm:
            if mark_task_complete(task_id):
                return DONE_HTML
            return "<h2>操作失败</h2>", 400
        return confirm_html(task_id, content, task_assignee)

    if assignee:
        tasks = get_tasks_by_assignee(assignee)
        return list_html(assignee, tasks)

    return "<p>Xplorify 任务提醒服务</p>"
